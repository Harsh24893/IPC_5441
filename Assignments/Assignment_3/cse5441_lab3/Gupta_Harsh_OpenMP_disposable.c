...
void computeDSV()
{
	...
	for (i = 0; i < num_grid_boxes; i++)
	{
		if(flag)
		{
			...
			if (id == 0)
			{
				NUM_OMP_THREAD = omp_get_num_threads();// to check actual number of threads created by OpenMP
				flag = false;
			}
		}
...
int main(int argc, char* argv[])
{
	...
	//MAIN LOOP-repeat until converged
	do {
		computeDSV();
		commitNewWeightedAvg();
		++numberOfIterations;
	} while (!isConvergenceAchieved());
	...
	printf("Number of OpenMP threads: %d, Affect rate = %lf , epsilon = %lf\n", NUM_OMP_THREAD, AFFECT_RATE, EPSILON);
